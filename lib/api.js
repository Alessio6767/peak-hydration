...
function createSession(db, subscriptionId) {
  const token = crypto.randomBytes(24).toString('hex');
  db.sessions = db.sessions || [];
  db.sessions.push({ token, subscriptionId, createdAt: Date.now() });
  return token;
}

function sessionSub(db, req) {
  const auth = req.headers.authorization || '';
  const token = auth.replace(/^Bearer\s+/i, '');
  if (!token) return null;
  const sess = (db.sessions || []).find((s) => s.token === token);
  return sess ? db.subscriptions.find((s) => s.id === sess.subscriptionId) : null;
}
...
function publicSubscription(sub) {
  ...
  return {
    ...
    deliveryFee: D.FREQUENCIES[sub.frequency].deliveryFee,
    totalPerDelivery: D.totalPerDelivery(sub.packageId, sub.frequency),
...
function scheduleCycle(db, sub, deliveryDate, today) {
  ...
  const debit = {
    ...
    amount: D.totalPerDelivery(sub.packageId, sub.frequency),
...
function createSubscription(body, today) {
  ...
  if (body && body.password !== undefined && String(body.password).length < 6)
  ...
  if (body.password !== undefined) sub.passwordHash = hashPassword(String(body.password));
  ...
  const token = body.password !== undefined ? createSession(db, sub.id) : undefined;
  store.save();
...
function stats(today) {
  ...
  const monthlyRevenue = active.reduce(
    (t, s) => t + D.totalPerDelivery(s.packageId, s.frequency) * D.FREQUENCIES[s.frequency].deliveriesPerMonth,
    0
  );
  ...
  try {
    ...
    if (pathname === '/api/login' && req.method === 'POST') {
      ...
      const matches = db.subscriptions
        .filter((s) => s.email === email && s.passwordHash)
        .sort((a, b) => b.id.localeCompare(a.id));
      const sub = matches.find((s) => checkPassword(password, s.passwordHash));
      if (!sub) return json(res, 401, { error: 'Email or password is incorrect' });
      const token = createSession(db, sub.id);
      store.save();
      return json(res, 200, { token });
    }

    if (pathname === '/api/me' && req.method === 'GET') {
      const sub = sessionSub(db, req);
      if (!sub) return json(res, 401, { error: 'Not logged in' });
      const deliveries = db.deliveries.filter((d) => d.subscriptionId === sub.id);
      const debits = db.debitOrders.filter((d) => d.subscriptionId === sub.id);
      const openDebit = debits.find((d) => d.status === 'pending' || d.status === 'failed');
      return json(res, 200, {
        subscription: publicSubscription(sub),
        upcomingDelivery: deliveries.find((d) => d.status === 'pending_payment' || d.status === 'scheduled') || null,
        openDebit: openDebit || null,
        transactions: db.transactions.filter((t) => t.subscriptionId === sub.id).reverse(),
        deliveryWindow: D.DELIVERY_WINDOW,
      });
    }

    if (pathname === '/api/me/pay' && req.method === 'POST') {
      const sub = sessionSub(db, req);
      if (!sub) return json(res, 401, { error: 'Not logged in' });
      if (sub.status === 'cancelled') return json(res, 409, { error: 'This subscription is cancelled' });
      const debit = db.debitOrders.find(
        (d) => d.subscriptionId === sub.id && (d.status === 'pending' || d.status === 'failed')
      );
      if (!debit) return json(res, 409, { error: 'Nothing to pay — you are all set' });
      const account = db.bankAccounts.find((a) => a.id === sub.bankAccountId);
      const delivery = db.deliveries.find((dl) => dl.id === debit.deliveryId);
      debit.attempts += 1;
      debit.processedAt = today;
      const ok = account && account.balance >= debit.amount;
      if (ok) {
        account.balance -= debit.amount;
        debit.status = 'paid';
      ...
      store.save();
      if (!ok) return json(res, 402, { error: 'The debit was declined: insufficient funds in the simulated account' });
      return json(res, 200, { paid: debit.amount, deliveryDate: delivery ? delivery.date : null });
    }
